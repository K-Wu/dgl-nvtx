...
G = dgl.heterograph({
        ('paper', 'written-by', 'author') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('author','writes','paper')][[1,0]]))))),
        ('author', 'writing', 'paper') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('author','writes','paper')]))))),
        ('paper', 'citing', 'paper') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('paper','cites','paper')]))))),
        ('paper', 'cited', 'paper') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('paper','cites','paper')][[1,0]]))))),
        ('paper', 'is-about', 'subject') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('paper','has_topic','field_of_study')]))))),
        ('subject', 'has', 'paper') : tuple(map(lambda x:x.flatten(),list(map(np.array,zip(dataset.graph['edge_index_dict'][('paper','has_topic','field_of_study')][[1,0]]))))),
    })
print(G)
