...
class HGTLayerMultiStream(nn.Module):
    def __init__(self,
                 in_dim,
                 out_dim,
                 node_dict,
                 ...
                 dropout=0.2,
                 use_norm=False):
        super(HGTLayerMultiStream, self).__init__()
...
class HGTMultiStream(nn.Module):
    def __init__(self, G, node_dict, edge_dict, n_inp, n_hid, n_out, n_layers, n_heads, use_norm=True):
        super(HGTMultiStream, self).__init__()
        self.node_dict = node_dict
        self.edge_dict = edge_dict
        self.gcs = nn.ModuleList()
        self.n_inp = n_inp
        ...
        for t in range(len(node_dict)):
            self.adapt_ws.append(nn.Linear(n_inp, n_hid))
        for _ in range(n_layers):
            self.gcs.append(HGTLayerMultiStream(n_hid, n_hid, node_dict, edge_dict, n_heads, use_norm=use_norm))
        self.out = nn.Linear(n_hid, n_out)
